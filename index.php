<!DOCTYPE html>
<html>
    <head>
        <meta charset="utf-8">
        <meta name="description" content="">
        <meta name="keyowrds" content="online learning, online student program, study chum, studychum, find students, students with same course">
        <meta name="viewport" content="width=device-width, initial-scale=1">
        <title>StudyChum, find people with similar learning interests and study together</title>
        <link rel="stylesheet" href='http://fonts.googleapis.com/css?family=Source+Sans+Pro:300,400,600'>
        <link rel="stylesheet" href="assets/css/bs.css">
        <link rel="stylesheet" href="assets/css/chum.css">
        <link rel="stylesheet" href="assets/css/index.css">
    </head>
    <body>
        <div class="container">
        	<div class="row">
                <div class="header">
                    <div class="chum">StudyChum logo</div>
                    <div class="tagline">Find people with similar learning interests and study together</div> 
                </div>
            </div>
        </div>
        <div class="info">
            <div class="mast">Find a mate or tutor</div>
            <div class="mast">Create a study group</div>
            <div class="mast">Create content as a tutor</div>
        </div>

        <div class="container">
            <div class="row signup">
                <div class="col-md-2"><a href="/learner"><img src="assets/img/signingoogle.webp" class="google" alt="Sign In With Google"></a></div>
            </div>
        </div>

        <script src="assets/js/jquery-2.0.3.min.js"></script>
        <script src="assets/js/chum.js"></script>
    </body>
</html>